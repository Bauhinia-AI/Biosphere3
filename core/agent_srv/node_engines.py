...
async def generate_daily_objective(state: RunningState):
    ...
    while retry_count < 3:
        try:
            obj_planner_prompt = state["prompts"]["obj_planner_prompt"]
            planner_response: RunningState = await obj_planner.ainvoke(
                {
                    ...
                    "daily_goal": obj_planner_prompt["daily_goal"],
                    "refer_to_previous": obj_planner_prompt["refer_to_previous"],
                    "life_style": obj_planner_prompt["life_style"],
                    "additional_requirements": obj_planner_prompt[
...
async def generate_meta_action_sequence(state: RunningState):
    meta_action_sequence_prompt = state["prompts"]["meta_action_sequence_prompt"]
    meta_action_sequence = await meta_action_sequence_planner.ainvoke(
        {
            ...
            "tool_functions": state["meta"]["tool_functions"],
            "locations": state["meta"]["available_locations"],
            "task_priority": meta_action_sequence_prompt["task_priority"],
            "max_actions": meta_action_sequence_prompt["max_actions"],
            "additional_requirements": meta_action_sequence_prompt[
                "additional_requirements"
            ],
        }
    )
...
async def adjust_meta_action_sequence(state: RunningState):
    ...
    meta_seq_adjuster_prompt = state["prompts"]["meta_seq_adjuster_prompt"]
    meta_action_sequence = await meta_seq_adjuster.ainvoke(
        {
            ...
            "replan_time_limits": meta_seq_adjuster_prompt["replan_time_limits"],
            "additional_requirements": meta_seq_adjuster_prompt[
                "additional_requirements"
            ],
        }
    )
...
async def reflect_and_summarize(state: RunningState):
    try:
        ...
        reflection_prompt = state["prompts"]["reflection_prompt"]
        reflection_input = {
            "past_objectives": past_objectives,
            "replan_history": replan_history,
            "character_stats": state["character_stats"],
            "focus_topics": reflection_prompt["focus_topic"],
            "depth_of_reflection": reflection_prompt["depth_of_reflection"],
            "additional_requirements": reflection_prompt[
                "additional_requirements"
            ],
        }
        ...
        logger.info(
            f"📝 User {state['userid']}: Generated reflection - {reflection.reflection}"
        )

        return {
            "decision": {
                "reflections": state.get("decision", {}).get("reflections", [])
                + [{"timestamp": timestamp, "content": reflection.reflection}]
            }
        }

    except Exception as e:
        logger.error(
            f"❌ User {state['userid']}: Error generating reflection - {str(e)}"
        )
        return {}
