...
# 质朴', '表达直接', '夸张幽默', '回味无穷', '典雅高贵'], 'biography': '生活在一座海港城市，自己从小对航海充满了好奇。希望能成为一名船长，掌控自己的船只。'}}
    ...
    response = make_api_request_sync("POST", "/sample/get", {"item_name": "personality"})
    print(response)
# {'code': 1, 'message': 'Sample retrieved successfully.', 'data': {'personality': ['开朗大方', '务实实际', '过分敏感']}}
    ...
    response = make_api_request_sync("POST", "/sample/get", {"item_name": "long_term_goal"})
    print(response)
# {'code': 1, 'message': 'Sample retrieved successfully.', 'data': {'long_term_goal': ['与其他智能体结成联盟', '撰写并发表突破性的研究论文']}}
    ...
    response = make_api_request_sync("POST", "/sample/get", {"item_name": "short_term_goal"})
    print(response)
# {'code': 1, 'message': 'Sample retrieved successfully.', 'data': {'short_term_goal': ['完成一次学术研究']}}
    ...
    response = make_api_request_sync("POST", "/sample/get", {"item_name": "language_style"})
    print(response)
